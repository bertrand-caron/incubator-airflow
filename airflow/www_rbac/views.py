...
# Custom OAuth user_info getter for Auth0.
# This method should be defined after `appbuilder` is defined, hence here.
@appbuilder.sm.oauth_user_info_getter
def auth0_oauth_user_info(sm, provider, response=None):
    if provider == 'auth0':
        logging.debug("Auth0 response received : {0}".format(response))

        AUTH_DOMAIN = conf.get('auth0', 'DOMAIN')
        user_info_url = 'https://%s/userinfo' % AUTH_DOMAIN

        resp = sm.oauth_remotes[provider].get(
            user_info_url, token=(response['access_token'], ''))
        if not resp or resp.status != 200:
            logging.error("Failed to fetch user profile, status({0})".format(
                resp.status if resp else 'None'))
            return {}

        logging.debug("UserInfo response {0}".format(resp.data))
        return {'username': 'auth0_' + resp.data['name'],
