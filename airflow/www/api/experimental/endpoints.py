...
@api_experimental.route('/dags/<string:dag_id>/dag_runs?state=<string:state>', methods=['GET'])
@requires_authentication
def dag_runs(dag_id, state):
    """
    Get all Dag Runs for given dag_id.
    """
    try:
        dagruns = get_dag_runs(dag_id, None if state == 'all' else state)
